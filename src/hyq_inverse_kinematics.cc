...
#include <xpp/hyqleg_inverse_kinematics.h>
...
//
//  int error_code;
//  bool successFlag = compute(leg, pos_b, q, error_code);
//
//  if(!successFlag) {
...
{
//  Joints q_xpp(kMapQuadEEToJoints.size(), GetJointsPerEE(), 0.0);
  ...
  for (auto ee : pos_b.GetEEsOrdered()) {
    ...
    Vector3d x = pos_b.At(ee);
    Vector3d q;
    mono::HyqlegInverseKinematics::KneeBend bend = mono::HyqlegInverseKinematics::Forward;

//    Vector3d q_LF, q_RF, q_LH, q_RF;

    switch (leg) {
      case LF:
        ee_pos_H = x + Eigen::Vector3d(-BASE2HAAX,-BASE2HAAY,-ZOFFSET);
        break;
      case RF:
        ee_pos_H = x.cwiseProduct(Eigen::Vector3d(1,-1,1)) + Eigen::Vector3d(-BASE2HAAX,-BASE2HAAY,-ZOFFSET);
        break;
      case LH:
        ee_pos_H = x.cwiseProduct(Eigen::Vector3d(-1,1,1)) + Eigen::Vector3d(-BASE2HAAX,-BASE2HAAY,-ZOFFSET);
...
//    if (leg == LF) {
//      ee_pos_H = x + Eigen::Vector3d(-BASE2HAAX,-BASE2HAAY,-ZOFFSET);
//      bend = mono::HyqlegInverseKinematics::Forward;
//    }
//    else if (leg == RF) {
//      ee_pos_H = x.cwiseProduct(Eigen::Vector3d(1,-1,1)) + Eigen::Vector3d(-BASE2HAAX,-BASE2HAAY,-ZOFFSET);
//      bend = mono::HyqlegInverseKinematics::Forward;
...
//  q.At(Reverse(kMapOptToQuad).at(LF)) = mono::HyqlegInverseKinematics::GetJointAngles(pos_b.At(Reverse(kMapOptToQuad).at(LF)) + Vector3d(-BASE2HAAX, -BASE2HAAY, -ZOFFSET));
//  q.At(Reverse(kMapOptToQuad).at(RF)) = mono::HyqlegInverseKinematics::GetJointAngles(pos_b.At(Reverse(kMapOptToQuad).at(RF)) + Vector3d(-BASE2HAAX,  BASE2HAAY, -ZOFFSET));
//  q.At(Reverse(kMapOptToQuad).at(LH)) = mono::HyqlegInverseKinematics::GetJointAngles(pos_b.At(Reverse(kMapOptToQuad).at(LH)) + Vector3d( BASE2HAAX, -BASE2HAAY, -ZOFFSET));
//  q.At(Reverse(kMapOptToQuad).at(RH)) = mono::HyqlegInverseKinematics::GetJointAngles(pos_b.At(Reverse(kMapOptToQuad).at(RH)) + Vector3d( BASE2HAAX,  BASE2HAAY, -ZOFFSET));
...
//HyqInverseKinematics::Joints1EE
//HyqInverseKinematics::GetUpperJointLimits (EEID ee) const
//{
//  // make sure to follow structure of GetJointAngles(). In this case
//  // HAA, HFE, KFE
//  Joints1EE q_max(DOF_PER_LEG);
//  switch (kMapOptToQuad.at(ee)) {
//    case LF:
...
//HyqInverseKinematics::Joints1EE
//HyqInverseKinematics::GetLowerJointLimits (EEID ee) const
//{
//  // make sure to follow structure of GetJointAngles(). In this case
//  // HAA, HFE, KFE
//  Joints1EE q_min(DOF_PER_LEG);
//  switch (kMapOptToQuad.at(ee)) {
//    case LF:
...
//	if (leg==RH || leg==LH) { // for hind legs swap the forward bent/rear bent
